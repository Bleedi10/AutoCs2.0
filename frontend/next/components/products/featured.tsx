      ...
      {hasAny ? (
        <div className="grid grid-cols-1 md:grid-cols-3  gap-10">
          <div className="md:col-span-2">
            {items[0] && <FeaturedItem product={items[0]} locale={locale} />}
          </div>
          <div className="grid gap-10">
            {items[1] && <FeaturedItem product={items[1]} locale={locale} />}
            {items[2] && <FeaturedItem product={items[2]} locale={locale} />}
          </div>
        </div>
      ) : (
        <div className="text-neutral-400">No hay productos destacados por ahora.</div>
      )}
