...
export const Pricing = ({
  ...
  const startSubscription = async (planCode?: string) => {
    ...
    try {
      ...
      if (!me?.authenticated) {
        window.location.href = `/${locale}/signup?plan=${planCode}`;
        return;
      }
      const resp = await fetch('/api/subscriptions/start', {
        method: 'POST',
  ...
  const onClick = (plan: any) => {
    if (plan?.CTA?.mode === 'subscribe') {
      return startSubscription(plan?.plan_code);
    }
    if (plan?.CTA?.href) {
      if (typeof window !== 'undefined') window.location.href = plan.CTA.href;
      return;
    }
  };
  return (
    <div className="pt-40">
      <Container>
        ...
        <Heading className="pt-4 bg-none text-neutral-900 dark:bg-clip-text dark:text-transparent dark:bg-gradient-to-b dark:from-neutral-800 dark:via-white dark:to-white">
          {heading}
        </Heading>
        <Subheading className="max-w-3xl mx-auto text-neutral-600 dark:text-muted">
          {sub_heading}
        </Subheading>
