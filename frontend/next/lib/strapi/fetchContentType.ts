...
export default async function fetchContentType(
  ...
  // Si no queremos usar Strapi (modo Django-only), devolvemos fallbacks sin hacer fetch.
  const useStrapi = String(process.env.NEXT_PUBLIC_USE_STRAPI || '').toLowerCase() === 'true';
  if (!useStrapi) {
  ...
  try {
    ...
    if (!response.ok) {
      // Reducimos ruido cuando Strapi no está disponible
      if (process.env.NODE_ENV !== 'production') {
        console.warn(
          `Strapi fetch fallback (url=${url.toString()}, status=${response.status})`
        );
      }
